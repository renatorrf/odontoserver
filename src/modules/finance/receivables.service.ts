...
function map(row: ReceivableRow) {
...
async function logRejectedFinancialOperation(auth: AuthContext, action: string, entityId: string, payload: Record<string, unknown>) {
  try {
    await query(`insert into odonto.audit_logs (empresa_id, usuario_id, entidade, entidade_id, acao, payload)
...
export async function listReceivables(auth: AuthContext, input: ReceivablesQuery) {
  const result = await query<ReceivableRow>(`${baseSql}
...
export async function receiveQuote(auth: AuthContext, quoteId: string, input: QuoteReceiptInput) {
  try {
    ...
    const existingOperation = await client.query<{ id: string; valor: string }>(
      `select id, valor::text from odonto.paciente_financeiro_pagamentos
    ...
    const quote = await client.query<{ paciente_id: string | null; nome_contato: string; desconto_valor: string }>(`
    ...
    if (!quote.rowCount) throw notFound('Orcamento nao encontrado.');
    const current = quote.rows[0];
    if (!current.paciente_id) throw badRequest('Vincule o orcamento a um paciente antes de receber.');
    const values = await client.query<{ bruto: string; cortesias: string; descontos_itens: string }>(`
    ...
    if (total <= 0) throw badRequest('O orcamento nao possui valor para recebimento.');
    await client.query(`select pg_advisory_xact_lock(hashtext($1), hashtext($2))`, [auth.empresaId, quoteId]);
    const operationAfterLock = await client.query<{ id: string; valor: string }>(
      `select id, valor::text from odonto.paciente_financeiro_pagamentos
        where empresa_id = $1 and idempotency_key = $2`, [auth.empresaId, input.idempotencyKey]);
    ...
    let entry = await client.query<{ id: string }>(`select id from odonto.paciente_financeiro_lancamentos
      where empresa_id = $1 and orcamento_id = $2 and status <> 'cancelado' order by created_at limit 1 for update`, [auth.empresaId, quoteId]);
    if (!entry.rowCount) {
      entry = await client.query<{ id: string }>(`insert into odonto.paciente_financeiro_lancamentos
        (empresa_id, paciente_id, orcamento_id, descricao, vencimento, valor, created_by, updated_by)
        values ($1, $2, $3, $4, current_date, $5, $6, $6) returning id`,
        [auth.empresaId, current.paciente_id, quoteId, `Orcamento #${quoteId.slice(0, 8).toUpperCase()} - ${current.nome_contato}`, total, auth.usuarioId]);
      titleCreated = true;
    }
    if (titleCreated) {
    ...
    const paid = await client.query<{ recebido: string; desconto: string; acrescimo: string }>(`select coalesce(sum(valor), 0)::text as recebido,
      coalesce(sum(desconto), 0)::text as desconto, coalesce(sum(acrescimo), 0)::text as acrescimo
      from odonto.paciente_financeiro_pagamentos where empresa_id = $1 and lancamento_id = $2 and estornado_em is null`,
      [auth.empresaId, entry.rows[0].id]);
    const received = Number(paid.rows[0].recebido);
    const balance = calculateFinancialPosition({ valor: total, recebido: received, desconto: Number(paid.rows[0].desconto), acrescimo: Number(paid.rows[0].acrescimo) }).saldo;
    if (input.desconto > balance) throw badRequest('O desconto excede o saldo do titulo.');
    if (input.valor + input.desconto > balance + input.acrescimo + 0.005) throw badRequest(`O valor excede o saldo de ${balance.toFixed(2)}.`);
    if (input.bancoId) {
      ...
      if (!bank.rowCount) throw badRequest('Conta bancaria invalida ou inativa.');
    }
    const payment = await client.query<{ id: string }>(`insert into odonto.paciente_financeiro_pagamentos
      ...
      values ($1, $2, $3, $4::odonto.paciente_forma_pagamento, $5, coalesce($6::timestamptz, now()), $7, $8, $9, $10, $11, $12, $13) returning id`,
      [auth.empresaId, entry.rows[0].id, input.valor, input.formaPagamento, input.parcelasCartao ?? null,
    ...
    const position = calculateFinancialPosition({ valor: total, recebido: newPaid, desconto: Number(paid.rows[0].desconto) + input.desconto, acrescimo: Number(paid.rows[0].acrescimo) + input.acrescimo });
...
export async function reversePayment(auth: AuthContext, paymentId: string, input: PaymentReversalInput) {
  try {
    ...
    if (!payment.rowCount) throw notFound('Recebimento nao encontrado.');
    if (payment.rows[0].estornado_em) throw badRequest('Este recebimento ja foi estornado.');
    const title = await client.query<{ id: string; paciente_id: string; orcamento_id: string | null; valor: string }>(`
    ...
    if (!title.rowCount) throw notFound('Titulo nao encontrado.');
    ...
    const totals = await client.query<{ recebido: string; desconto: string; acrescimo: string }>(`
    ...
    const position = calculateFinancialPosition({ valor: Number(title.rows[0].valor), recebido: Number(totals.rows[0].recebido),
