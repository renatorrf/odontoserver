...
import {
  ReturnAlertInput,
  ReturnAlertQuery,
  ReturnAlertStatusInput,
  ScheduleEventInput,
  ScheduleEventStatusInput,
  ScheduleQuery,
} from './schedule.schemas';
import { validateProfessionalSlot } from './availability.service';
...
interface EventRow {
  ...
  observacoes_procedimentos: string | null;
  observacoes_procedimentos_updated_at: string | null;
  observacoes_procedimentos_responsavel: string | null;
  inicio_em: string;
  fim_em: string;
  dia_inteiro: boolean;
...
async function validateReferences(
...
): Promise<{ patientName: string | null; patientWhatsapp: string | null }> {
  let patientName: string | null = null;
  let patientWhatsapp: string | null = null;

  if (input.profissionalId) {
    const professional = await client.query(
      `select 1 from odonto.profissionais where id = $1 and empresa_id = $2 and status = 'ativo' limit 1`,
      [input.profissionalId, auth.empresaId],
    );
    if (!professional.rowCount) {
      throw notFound('Profissional ativo nao encontrado.');
    }
  }

  if (input.pacienteId) {
    const patient = await client.query<{ nome: string; celular: string | null }>(
      `select pac.nome, pct.celular
    ...
    if (!patient.rowCount) {
      throw notFound('Paciente ativo nao encontrado.');
    }
  ...
  if (input.tipo === 'consulta') {
    ...
    if (whatsappDigits.length < 8 || whatsappDigits.length > 15) {
      throw badRequest('O paciente precisa ter um WhatsApp valido para gerar o orcamento da consulta.');
    }
  }
...
function notificationSettings(input: ScheduleEventInput) {
...
async function resolveEventPeriod(
  client: PoolClient,
  auth: AuthContext,
  input: ScheduleEventInput,
  ...
  if (input.diaInteiro) {
    ...
    const catalog = await client.query<{ id: string; duracao_minutos: number }>(
      `select id, duracao_minutos from odonto.catalogo_procedimentos
    ...
    const duration = catalog.rows.reduce(
      (total, procedure) => total + procedure.duracao_minutos * (quantities.get(procedure.id) ?? 1),
      0,
    );
...
async function syncConsultationQuote(
  ...
  const eventResult = await client.query<{ orcamento_id: string | null }>(
    `select orcamento_id
  ...
  if (quoteId) {
    const updated = await client.query(
      `update odonto.orcamentos
        ...
        where id = $1 and empresa_id = $2`,
      [quoteId, auth.empresaId, input.pacienteId, input.profissionalId, patientName,
        patientWhatsapp, optionalText(input.observacoesProcedimentos), auth.usuarioId],
    );
    if (!updated.rowCount) {
      quoteId = null;
    }
  ...
  if (!quoteId) {
    const created = await client.query<{ id: string }>(
      `insert into odonto.orcamentos (
       ...
       returning id`,
      [auth.empresaId, input.pacienteId, input.profissionalId, patientName, patientWhatsapp,
        optionalText(input.observacoesProcedimentos), auth.usuarioId],
    );
    quoteId = created.rows[0].id;
    await client.query(
      `update odonto.agenda_eventos set orcamento_id = $3
        where id = $1 and empresa_id = $2`,
      [eventId, auth.empresaId, quoteId],
    );
  }
...
function mapEvent(row: EventRow, auth: AuthContext) {
  ...
  const alertLabels = canViewClinical
    ? row.anamnese_alertas.map((code) => anamnesisAlertLabels[code]).filter(Boolean)
    : [];
...
export async function listEvents(auth: AuthContext, input: ScheduleQuery) {
...
export async function getEvent(auth: AuthContext, id: string) {
  const result = await query<EventRow>(
    `${eventSelect}
...
export async function createEvent(auth: AuthContext, input: ScheduleEventInput) {
  try {
    const id = await transaction(async (client) => {
      ...
      const result = await client.query<{ id: string }>(
        ...
        [
          ...
          title, optionalText(input.categoria), optionalText(input.observacoes), optionalText(input.observacoesProcedimentos),
          period.start.toISOString(), period.end.toISOString(), input.diaInteiro, input.primeiraConsulta, optionalText(input.confirmacaoEnvio),
          optionalText(input.lembreteEnvio), notifications.enabled, notifications.app, notifications.whatsapp,
          auth.usuarioId,
        ],
      );
      await replaceProcedures(client, auth, result.rows[0].id, input.procedimentos);
      if (input.tipo === 'consulta') {
        await syncConsultationQuote(
          client,
          auth,
          result.rows[0].id,
          input,
      ...
      await client.query(
        ...
        [auth.empresaId, result.rows[0].id, auth.usuarioId],
      );
      if (optionalText(input.observacoesProcedimentos)) {
        await client.query(
          `insert into odonto.audit_logs (empresa_id, usuario_id, entidade, entidade_id, acao, payload)
           values ($1, $2, 'agenda_eventos', $3, 'create_procedure_notes', $4::jsonb)`,
          [auth.empresaId, auth.usuarioId, result.rows[0].id, JSON.stringify({ campo: 'observacoes_procedimentos' })],
        );
      }
    ...
    if (event.tipo === 'consulta') {
      await sendScheduleNotification(id, 'confirmacao_agendamento', auth.usuarioId);
    }
    return event;
  } catch (error: unknown) {
...
export async function updateEvent(auth: AuthContext, id: string, input: ScheduleEventInput) {
  try {
    ...
    await transaction(async (client) => {
      ...
      const title = input.tipo === 'consulta' ? patientName! : input.titulo!;
      const procedureNotes = optionalText(input.observacoesProcedimentos);
      ...
      await client.query(
        ...
        [
          ...
          title, optionalText(input.categoria), optionalText(input.observacoes), procedureNotes, period.start.toISOString(),
          period.end.toISOString(), input.diaInteiro, input.primeiraConsulta, optionalText(input.confirmacaoEnvio),
          optionalText(input.lembreteEnvio), notifications.enabled, notifications.app, notifications.whatsapp,
      ...
      if (input.tipo === 'consulta') {
        ...
        await client.query(
          'update odonto.agenda_eventos set orcamento_id = null where id = $1 and empresa_id = $2',
          [id, auth.empresaId],
        );
      }
      if (procedureNotesChanged) {
        await client.query(
          `insert into odonto.audit_logs (empresa_id, usuario_id, entidade, entidade_id, acao, payload)
           values ($1, $2, 'agenda_eventos', $3, 'update_procedure_notes', $4::jsonb)`,
          [auth.empresaId, auth.usuarioId, id, JSON.stringify({ campo: 'observacoes_procedimentos' })],
        );
      }
      if (rescheduled) {
        await client.query(
          ...
          [auth.empresaId, id, previous.inicio_em, previous.fim_em, period.start.toISOString(),
            period.end.toISOString(), optionalText(input.motivoRemarcacao), auth.usuarioId],
        );
      }
    });
    const event = await getEvent(auth, id);
...
export async function updateEventStatus(
  auth: AuthContext,
  id: string,
  input: ScheduleEventStatusInput,
): Promise<void> {
  await transaction(async (client) => {
    const currentResult = await client.query<{ status: ScheduleStatus }>(
      'select status::text from odonto.agenda_eventos where id = $1 and empresa_id = $2 for update',
      [id, auth.empresaId],
    );
    ...
    const allowed = scheduleStatusTransitions[current] ?? [];
    ...
    await client.query(
      `update odonto.agenda_eventos set
         ...
         updated_by = $5
       where id = $1 and empresa_id = $2`,
      [id, auth.empresaId, input.status, optionalText(input.justificativa), auth.usuarioId],
    );
    await client.query(
      `insert into odonto.agenda_evento_status_historico (
...
export async function createReturnAlert(auth: AuthContext, input: ReturnAlertInput) {
  const alertId = await transaction(async (client) => {
    ...
    const result = await client.query<{ id: string }>(
      ...
      [
        ...
        input.retornarEm, optionalText(input.observacoes), auth.usuarioId,
      ],
    );
    return result.rows[0].id;
  });
...
export async function updateReturnAlertStatus(
  auth: AuthContext,
  id: string,
  input: ReturnAlertStatusInput,
): Promise<void> {
  ...
  if (!result.rowCount) {
    throw notFound('Alerta de retorno nao encontrado.');
  }
}
