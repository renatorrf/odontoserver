...
exports.getFinanceStatement = getFinanceStatement;
exports.billProcedures = billProcedures;
...
function commissionValue(procedureValue, commission) {
    if (!commission) {
        return 0;
    }
...
async function findCommission(client, empresaId, profissionalId, procedureDate) {
...
async function getFinanceStatement(auth, input) {
...
async function billProcedures(auth, input) {
    ...
    return (0, pool_1.transaction)(async (client) => {
      ...
      `, [auth.empresaId, uniqueIds]);
        if (result.rows.length !== uniqueIds.length) {
            throw (0, http_error_1.notFound)('Um dos procedimentos realizados nao foi encontrado.');
        }
        if (result.rows.some((row) => !row.profissional_id)) {
            throw (0, http_error_1.conflict)('Vincule todos os procedimentos a um profissional antes de faturar.');
        }
        if (result.rows.some((row) => row.lancamento_id)) {
            throw (0, http_error_1.conflict)('Um dos procedimentos selecionados ja foi faturado.');
        }
        for (const procedure of result.rows) {
            const commission = await findCommission(client, auth.empresaId, procedure.profissional_id, procedure.data_procedimento);
