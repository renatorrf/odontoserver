...
exports.updateExpensePayment = updateExpensePayment;
exports.getOperationalCost = getOperationalCost;
exports.saveOperationalCostConfig = saveOperationalCostConfig;
exports.getSimplifiedDre = getSimplifiedDre;
...
async function ensureBank(client, auth, id) {
    if (!id) {
        return;
    }
    ...
    if (!result.rowCount) {
        throw (0, http_error_1.notFound)('Conta bancaria nao encontrada.');
    }
}
async function insertOccurrence(client, auth, input, seriesId, dueDate, occurrence, competence = monthStart(dueDate)) {
    ...
    `, [
        ...
        input.descricao,
        input.categoria,
        (0, normalize_1.optionalText)(input.fornecedor),
        (0, normalize_1.optionalText)(input.centroCusto),
        (0, normalize_1.optionalText)(input.documento),
        competence,
        dueDate,
        input.valor,
...
async function getExpenseRow(auth, id) {
...
async function listBanks(auth) {
    const result = await (0, pool_1.query)('select * from odonto.bancos where empresa_id = $1 order by ativo desc, nome', [auth.empresaId]);
    return result.rows.map(mapBank);
}
async function createBank(auth, input) {
    try {
        const result = await (0, pool_1.query)(`
      ...
      `, [
            auth.empresaId,
            input.nome,
            (0, normalize_1.optionalText)(input.codigoBanco),
            (0, normalize_1.optionalText)(input.agencia),
            (0, normalize_1.optionalText)(input.conta),
            input.tipoConta,
            (0, normalize_1.optionalText)(input.titular),
            (0, normalize_1.optionalText)(input.documentoTitular),
        ...
        if (error.code === '23505') {
            throw (0, http_error_1.conflict)('Ja existe uma conta bancaria com este nome.');
        }
        throw error;
    }
}
async function updateBank(auth, id, input) {
    try {
        const result = await (0, pool_1.query)(`
...
async function listExpenses(auth, input) {
    const result = await (0, pool_1.query)(`
      select d.*, b.nome as banco_nome, ds.recorrencia::text, ds.fim_em as recorrencia_fim
        from odonto.despesas d
        left join odonto.bancos b on b.id = d.banco_id and b.empresa_id = d.empresa_id
...
async function createExpense(auth, input) {
    const id = await (0, pool_1.transaction)(async (client) => {
      ...
      `, [
        ...
        while (current <= limit && occurrence <= 120) {
            const occurrenceId = await insertOccurrence(client, auth, input, seriesId, dateText(current), occurrence, occurrence === 1 ? input.competencia : monthStart(dateText(current)));
...
async function updateExpense(auth, id, input) {
    await (0, pool_1.transaction)(async (client) => {
        await ensureBank(client, auth, input.bancoId);
        const result = await client.query('select serie_id, status::text, vencimento from odonto.despesas where id = $1 and empresa_id = $2 for update', [id, auth.empresaId]);
        const current = result.rows[0];
        if (!current) {
            throw (0, http_error_1.notFound)('Despesa nao encontrada.');
        }
        if (current.status === 'paga') {
            throw (0, http_error_1.conflict)('Reabra a despesa como pendente antes de altera-la.');
        }
        if (input.aplicarProximas && !current.serie_id) {
            throw (0, http_error_1.conflict)('Esta despesa nao pertence a uma recorrencia.');
        }
        const values = [
            input.bancoId ?? null,
            input.descricao,
...
async function cancelExpense(auth, id, applyNext) {
    return (0, pool_1.transaction)(async (client) => {
        ...
        if (current.status === 'paga') {
            throw (0, http_error_1.conflict)('Despesas pagas nao podem ser excluidas. Reabra o pagamento primeiro.');
        }
        if (applyNext && current.serie_id) {
            const canceled = await client.query(`update odonto.despesas set status = 'cancelada', updated_by = $4
          where empresa_id = $1 and serie_id = $2 and status = 'pendente' and vencimento >= $3::date`, [auth.empresaId, current.serie_id, current.vencimento, auth.usuarioId]);
        ...
        await client.query(`update odonto.despesas set status = 'cancelada', updated_by = $3 where id = $1 and empresa_id = $2`, [id, auth.empresaId, auth.usuarioId]);
        return 1;
    });
}
async function updateExpensePayment(auth, id, input) {
    await (0, pool_1.transaction)(async (client) => {
      ...
      `, [
            ...
            (0, normalize_1.optionalText)(input.referenciaPagamento),
            (0, normalize_1.optionalText)(input.observacoes),
            input.pagaEm ?? null,
            auth.usuarioId,
        ]);
...
async function getOperationalCost(auth, input) {
    const [expenseResult, configResult, procedureResult] = await Promise.all([
      ...
      `, [auth.empresaId, input.inicio, input.fim]),
        (0, pool_1.query)('select quantidade_cadeiras, horas_produtivas_cadeira_mes from odonto.custo_operacional_config where empresa_id = $1', [auth.empresaId]),
        (0, pool_1.query)(`select id, nome, categoria, duracao_minutos, valor::text, custo_variavel::text
         from odonto.catalogo_procedimentos where empresa_id = $1 and ativo = true order by nome`, [auth.empresaId]),
    ]);
...
async function saveOperationalCostConfig(auth, input) {
...
async function getSimplifiedDre(auth, input) {
...
async function getOperationalResults(auth, input) {
