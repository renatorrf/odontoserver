...
exports.listRedFolder = listRedFolder;
exports.sendRetentionContact = sendRetentionContact;
...
async function listRedFolder(auth, input) {
    const inactiveResult = input.tipo === 'orcamentos' ? { rows: [] } : await (0, pool_1.query)(`
      select
    ...
    const lostQuoteResult = input.tipo === 'inativos' ? { rows: [] } : await (0, pool_1.query)(`
    ...
    const quoteOpportunities = lostQuoteResult.rows.map((row) => ({
        ...
        tipo: 'orcamento_nao_aprovado',
        perfil: row.paciente_id ? 'paciente' : 'novo',
        pacienteId: row.paciente_id,
...
async function sendRetentionContact(auth, patientId, input) {
    const patientResult = await (0, pool_1.query)(`
    ...
    if (input.procedimentoId && !patient.procedimento_nome) {
        throw (0, http_error_1.notFound)('Procedimento ofertado nao encontrado.');
    }
