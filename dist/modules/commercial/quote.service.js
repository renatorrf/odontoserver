...
async function validateQuote(auth, input, client) {
    ...
    if (hasFinancialBenefit && !['portal_admin', 'gestor'].includes(auth.perfil)) {
        throw (0, http_error_1.forbidden)('Somente gestores podem conceder descontos ou cortesias.');
    }
    if (input.pacienteId) {
        ...
        if (!patient.rowCount) {
            throw (0, http_error_1.notFound)('Paciente vinculado nao encontrado.');
        }
    }
    ...
    if (discount > subtotal) {
        throw (0, http_error_1.badRequest)('O desconto nao pode ser maior que o subtotal do orcamento.');
    }
    return items;
}
...
async function createQuote(auth, input) {
    const quoteId = await (0, pool_1.transaction)(async (client) => {
      ...
      values ($1,$2,'orcamento',$3,'orcamento_criado',$4::jsonb)`, [auth.empresaId, auth.usuarioId, result.rows[0].id,
        ...
        if (input.descontoValor > 0 || items.some((item) => item.cortesia || item.descontoValor > 0))
            await client.query(`insert into odonto.audit_logs (empresa_id, usuario_id, entidade, entidade_id, acao, payload)
...
async function updateQuoteStatus(auth, quoteId, input) {
    await (0, pool_1.transaction)(async (client) => {
        const current = await client.query('select status::text from odonto.orcamentos where id = $1 and empresa_id = $2 for update', [quoteId, auth.empresaId]);
        if (!current.rowCount)
            throw (0, http_error_1.notFound)('Orcamento nao encontrado.');
...
async function sendQuote(auth, quoteId, input) {
    const destinationResult = await (0, pool_1.query)(`
    ...
    if (channels.includes('aplicativo') && !quote.paciente_id) {
        throw (0, http_error_1.badRequest)('Vincule um paciente antes de enviar o orcamento pelo aplicativo.');
    }
    ...
    await (0, pool_1.transaction)(async (client) => {
        if (appStatus && quote.paciente_id) {
          ...
          'portal do paciente', 'enviada', now(), $5)`, [auth.empresaId, quote.paciente_id, quote.id, message, auth.usuarioId]);
        }
        if (whatsappStatus) {
            const phone = normalizeWhatsAppNumber(quote.whatsapp);
            await client.query(`insert into odonto.notificacoes (
          ...
          $6, $7, $8, $9, $10)`, [auth.empresaId, quote.paciente_id, quote.id, message, phone, whatsappStatus,
                whatsappProviderId, whatsappError, whatsappStatus === 'enviada' ? new Date() : null, auth.usuarioId]);
        }
        await client.query(`insert into odonto.comercial_contatos (
        empresa_id, paciente_id, orcamento_id, mensagem, canais,
        aplicativo_status, whatsapp_status, created_by
      ) values ($1, $2, $3, $4, $5::varchar[], $6, $7, $8)`, [auth.empresaId, quote.paciente_id, quote.id, message, channels, appStatus, whatsappStatus, auth.usuarioId]);
        await client.query(`update odonto.orcamentos set status = 'enviado', enviado_em = now(), updated_by = $3
        where id = $1 and empresa_id = $2`, [quote.id, auth.empresaId, auth.usuarioId]);
    });
    return {
        mensagem: message,
...
async function approveAndScheduleQuote(auth, quoteId, input) {
    try {
        const result = await (0, pool_1.transaction)(async (client) => {
            const quoteResult = await client.query(`select paciente_id, nome_contato, whatsapp, observacoes, status::text
           from odonto.orcamentos
          where id = $1 and empresa_id = $2
          limit 1 for update`, [quoteId, auth.empresaId]);
            const quote = quoteResult.rows[0];
            if (!quote)
                throw (0, http_error_1.notFound)('Orcamento nao encontrado.');
            if (['cancelado', 'expirado'].includes(quote.status)) {
                throw (0, http_error_1.badRequest)('Este orcamento nao pode ser aprovado.');
            }
            const existingEvent = await client.query(`select 1 from odonto.agenda_eventos
          where empresa_id = $1 and orcamento_id = $2 and status <> 'cancelado' limit 1`, [auth.empresaId, quoteId]);
            if (existingEvent.rowCount) {
                throw (0, http_error_1.conflict)('Este orcamento ja possui uma consulta agendada.');
            }
            const patient = await (0, patient_service_1.createOrLinkMinimalPatient)(client, auth, {
           ...
           from odonto.orcamento_itens where orcamento_id = $1 order by ordem, created_at`, [quoteId]);
            ...
            const duration = itemsResult.rows.reduce((total, item) => total + item.duracao_minutos * item.quantidade, 0);
            ...
            const eventResult = await client.query(`insert into odonto.agenda_eventos (
        ...
        returning id`, [auth.empresaId, input.profissionalId, patient.id, quoteId, patient.nome, quote.observacoes,
          ...
          where id = $1 and empresa_id = $2`, [quoteId, auth.empresaId, patient.id, input.profissionalId, auth.usuarioId]);
            return { eventId, patient };
        });
        const [orcamento, evento, notificacoes] = await Promise.all([
            getQuote(auth, quoteId),
            (0, schedule_service_1.getEvent)(auth, result.eventId),
            (0, schedule_notification_service_1.sendScheduleNotification)(result.eventId, 'confirmacao_agendamento', auth.usuarioId),
        ]);
        ...
        if (error.code === '23505') {
            throw (0, http_error_1.conflict)('CPF ja cadastrado ou orcamento ja agendado.');
        }
        throw error;
    }
}
